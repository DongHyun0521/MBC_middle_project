...
VALUES ('user01', 'aaaa!A', '김회원', 19901123, 1, '서울시 강남구 XX오피스텔', '108호', '010-1453-7897', 'user01@naver.com');

-- 회원 2
INSERT INTO mem (id, password, name, birthday, gender, address, address_detail, phone_number, email)
VALUES ('user02', 'aaaa!A', '이회원', 19850728, 2, '서울시 중구 XX빌라', '304호', '010-2734-8905', 'user02@kakao.com');

-- 회원 3
INSERT INTO mem (id, password, name, birthday, gender, address, address_detail, phone_number, email)
VALUES ('user03', 'aaaa!A', '박회원', 20010521, 1, '서울시 서초구 OO오피스텔', '1104호', '010-0324-5843', 'user03@gmail.com');
...
WITH inserted_admin AS (
    INSERT INTO mem (id, password, name, birthday, gender, address, address_detail, phone_number, email)
    VALUES ('admin31', 'aaaa!A', '김원무', 19700829, 2, '서울시 성북구 KK빌라', '404호', '010-3475-1048', 'admin31@shospital.com')
    RETURNING mem_id
)
...
WITH inserted_admin AS (
    INSERT INTO mem (id, password, name, birthday, gender, address, address_detail, phone_number, email)
    VALUES ('admin32', 'aaaa!A', '이원무', 19910923, 1, '서울시 강북구 PP아파트', '1104호', '010-1254-6941', 'admin32@shospital.com')
    RETURNING mem_id
)
INSERT INTO admin_staff (mem_id, rank, emp_number, status, admin_dept_id)
...
WITH inserted_admin AS (
    INSERT INTO mem (id, password, name, birthday, gender, address, address_detail, phone_number, email)
    VALUES ('admin33', 'aaaa!A', '박원무', 19870421, 1, '서울시 노원구 MM오피스텔', '2104호', '010-1474-5082', 'admin33@shospital.com')
    RETURNING mem_id
)
INSERT INTO admin_staff (mem_id, rank, emp_number, status, admin_dept_id)
...
WITH inserted_admin AS (
    INSERT INTO mem (id, password, name, birthday, gender, address, address_detail, phone_number, email)
    VALUES ('admin71', 'aaaa!A', '김홍보', 19921204, 1, '서울시 광진구 WW아파트', '205호', '010-3643-4587', 'admin71@shospital.com')
    RETURNING mem_id
)
INSERT INTO admin_staff (mem_id, rank, emp_number, status, admin_dept_id)
SELECT mem_id, '주임', 'A00000101', '재직', 7
FROM inserted_admin;

-- 행정:홍보 2
WITH inserted_admin AS (
    INSERT INTO mem (id, password, name, birthday, gender, address, address_detail, phone_number, email)
    VALUES ('admin72', 'aaaa!A', '이홍보', 19940929, 2, '서울시 중랑구 TT아파트', '1503호', '010-1465-4954', 'admin72@shospital.com')
    RETURNING mem_id
)
INSERT INTO admin_staff (mem_id, rank, emp_number, status, admin_dept_id)
SELECT mem_id, '대리', 'A00000101', '재직', 7
FROM inserted_admin;

-- 행정:홍보 3
WITH inserted_admin AS (
    INSERT INTO mem (id, password, name, birthday, gender, address, address_detail, phone_number, email)
    VALUES ('admin73', 'aaaa!A', '박홍보', 19890726, 2, '서울시 동대문구 XX아파트', '908호', '010-0645-1490', 'admin73@shospital.com')
    RETURNING mem_id
)
INSERT INTO admin_staff (mem_id, rank, emp_number, status, admin_dept_id)
SELECT mem_id, '팀장', 'A00000101', '재직', 7
FROM inserted_admin;
...
(1, '병실 와이파이가 자주 끊깁니다.', '별관 5층 병동에 입원 중인 환자입니다. 공용 와이파이가 자꾸 끊겨서 동영상 시청이나 가족들과의 영상 통화가 불편합니다. 공유기를 점검해 주시거나 추가 설치 부탁드립니다.', NULL, now() - INTERVAL '15 days'),
(1, '환자식(일반식) 간이 너무 짜게 나오는 것 같습니다.', '병원 밥은 싱거울 줄 알았는데, 최근 며칠 동안 나온 반찬들(특히 조림류)이 제 입맛에는 너무 짭니다. 환자들의 건강을 위해 나트륨 함량을 조금 더 줄여주시면 어떨까요?', '/images/voc_04.jpg', now() - INTERVAL '14 days'),
(1, '건강검진 예약 날짜를 부득이하게 변경하고 싶습니다.', '다음 주 화요일에 종합건강검진이 예약되어 있는데, 회사 출장 일정이 갑자기 잡혀서 2주 뒤로 미루고 싶습니다. 온라인에서는 취소만 보이고 변경 버튼이 안 보이는데 어떻게 해야 하나요?', NULL, now() - INTERVAL '13 days'),
(1, '응급실 보안요원 분의 발 빠른 조치에 감사드립니다.', '새벽에 아이가 고열이 나서 응급실에 뛰어갔는데, 당황해서 어쩔 줄 모르는 저를 안심시켜 주시고 접수부터 안내까지 곁에서 도와주신 보안요원분 정말 감사합니다. 덕분에 무사히 치료받았습니다.', '/images/voc_05.jpg', now() - INTERVAL '12 days'),
(1, '로비 화장실 휴지통이 너무 빨리 찹니다.', '본관 1층 로비 화장실을 이용했는데, 휴지통이 꽉 차서 주변에 쓰레기가 넘치고 있었습니다. 방문객이 많은 층인 만큼 미화원분들께서 조금 더 자주 비워주셨으면 좋겠습니다.', NULL, now() - INTERVAL '11 days'),
(1, '채혈실에서 피 뽑을 때 멍이 너무 심하게 들었습니다.', '어제 채혈을 했는데 간호사님이 혈관을 잘 못 찾으셔서 여러 번 찔렀고, 결국 오늘 보니까 팔 전체에 시퍼렇게 멍이 들었습니다. 숙련된 분이 채혈해 주셨으면 합니다.', '/images/voc_06.jpg', now() - INTERVAL '10 days'),
...
(3, '소아청소년과 선생님, 아이 눈높이 진료 감사합니다.', '병원만 오면 자지러지게 우는 아이인데, 청진기 대기 전에 인형으로 장난도 쳐주시고 스티커도 주시면서 달래주신 과장님 덕분에 울지 않고 무사히 진료 마쳤습니다. 최고예요!', '/images/voc_09.jpg', now() - INTERVAL '4 days'),
(3, '휠체어 전용 엘리베이터를 지정해 주시면 좋겠습니다.', '환자 이동 침대나 휠체어를 탄 환자들이 층간 이동을 할 때 일반 방문객들과 엘리베이터가 섞여서 타기가 너무 힘듭니다. 환자 전용 탑승기를 지정해서 운영해 주세요.', NULL, now() - INTERVAL '3 days'),
(3, '층별 안내판이 너무 헷갈리게 되어 있습니다.', '본관에서 별관으로 넘어가는 연결 통로를 찾기가 너무 어렵습니다. 바닥에 색깔 선을 그어주거나 표지판 글씨를 지금보다 훨씬 크게 만들어 주셨으면 합니다.', '/images/voc_10.jpg', now() - INTERVAL '2 days'),
(3, '모바일 앱에서 진료 이력 조회가 안 됩니다.', '안드로이드 휴대폰 사용 중인데, 로그인하고 나의 진료내역 메뉴를 누르면 하얀 화면만 뜨고 멈춰버립니다. 앱 오류인 것 같은데 확인 및 조치 부탁드립니다.', NULL, now() - INTERVAL '1 day');
