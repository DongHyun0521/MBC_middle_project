    ...
    <div class="nurse-dash-wrapper">
            ...
            <div class="section-card">
                <div class="card-head">
                    ...
                    <div class="toggle-group">
                        <button :class="{ active: nurViewMode === 'calendar' }" @click="nurViewMode = 'calendar'">달력</button>
                ...
                <div v-if="nurViewMode === 'calendar'" class="calendar-wrap">
                    <div class="cal-header">
                        <button @click="changeMonth(-1)"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 18l-6-6 6-6" /></svg></button>
                        <h4>{{ calYear }}년 {{ calMonth }}월</h4>
                        <button @click="changeMonth(1)"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 18l6-6-6-6" /></svg></button>
                    </div>
                    <div class="cal-grid">
                    ...
                    <div v-if="selectedDate" class="selected-date-list">
                        ...
                        <table class="hospital-tbl mt-10">
                            <thead><tr><th>시간</th><th>환자명</th><th>상태</th></tr></thead>
                            <tbody>
                ...
                <div v-else>
                    <div class="filter-tabs">
                        <button v-for="st in ['전체', '예약', '완료', '취소']" :key="st"
                            :class="['filter-btn', { active: nurFilter === st }]" @click="nurFilter = st">{{ st }}</button>
                    </div>
                    <table class="hospital-tbl">
                        <thead><tr><th>날짜</th><th>시간</th><th>환자명</th><th>상태</th></tr></thead>
                        <tbody>
        ...
        <div v-if="currentView === 'nur_schedule'" class="view-section">
            <div class="section-card">
                ...
                <div class="calendar-wrap">
                    ...
                    <div class="cal-grid">
                        <div v-for="day in ['일', '월', '화', '수', '목', '금', '토']" :key="day" class="cal-day-head">{{ day }}</div>
                        <div v-for="(date, idx) in calendarDays" :key="idx" :class="['cal-cell', { 'diff-month': !date.isCurrentMonth, 'today': isToday(date.fullDate) }]">
                            <span class="day-num">{{ date.day }}</span>
                            <div class="cal-events">
                                <div v-if="getShiftForDate(date.fullDate)" :class="['shift-badge', getShiftClass(getShiftForDate(date.fullDate))]">
        ...
        <div v-if="currentView === 'nur_ward'" class="view-section">
            <div class="section-card">
                ...
                <table class="hospital-tbl">
                    <thead><tr><th>호실</th><th>환자명</th><th>입원일</th><th>상태</th></tr></thead>
                    <tbody>
        ...
        <div v-if="currentView === 'res'" class="view-section">
            <div class="section-card">
                <div class="card-head"><h3>내 진료 예약 내역</h3></div>
                <div class="filter-tabs">
...
/* 📌 2. 섹션 카드 & 헤더 */
.dash-card, .section-card {
    background: #ffffff;
    border: 1px solid #eee;
    padding: 40px; /* [수치보존] 공주님 지정값 */
    margin-bottom: 20px;
...
/* 📌 3. 정보 리스트 스타일 */
.info-list .info-item { font-size: 17px; margin-bottom: 12px; }
.info-item .label { width: 100px; color: #888; font-weight: 700; font-size: 20px; padding: 8px; }
.val { padding: 8px 8px 16px; font-size: 18px; margin-bottom: 18px; border-bottom: 1px solid #f5f5f5; }

/* 📌 4. 현황 통계 (Stat Box) */
.stat-grid { display: flex; gap: 20px; }
.stat-box {
    flex: 1; padding: 30px 20px; background-color: #f8f9fa; border: 1px solid #eee;
    text-align: center; font-size: 18px; font-weight: 600; color: #666;
}
.stat-box span { display: block; font-size: 40px; font-weight: 800; color: #333; margin-bottom: -12px; }
.stat-box.blue span { color: #005baa; }

/* 📌 5. 병원 공통 테이블 */
.hospital-tbl { width: 100%; border-collapse: collapse; margin-top: 10px; }
.hospital-tbl th { background-color: #f8f9fa; padding: 30px; text-align: left; font-weight: 600; color: #555; border-bottom: 2px solid #eee; font-size: 18px; }
.hospital-tbl td { padding: 18px 30px; border-bottom: 1px solid #f0f0f0; color: #444; font-size: 18px; vertical-align: middle; }
.bold-blue { color: #005baa; font-weight: 700; }

/* 📌 6. 필터 및 버튼 디자인 */
.filter-tabs, .toggle-group { display: flex; gap: 12px; margin-bottom: 25px; }
.filter-btn, .toggle-group button {
    padding: 10px 20px; background-color: #fff; border: 1px solid #ddd;
    color: #888; font-size: 16px; font-weight: 600; cursor: pointer; transition: all 0.2s;
}
.filter-btn.active, .toggle-group button.active { background-color: #005baa; color: #fff; border-color: #005baa; }
.btn-cancel-table { background: #fff; color: #888; border: 1px solid #ddd; padding: 10px 20px; font-weight: 600; cursor: pointer; font-size: 16px; }
.btn-cancel-table:hover { background: #f8f8f8; color: #dc3545; border-color: #dc3545; }

/* 📌 7. 달력 레이아웃 (💡 공주님 커스텀 위치 적용) */
.calendar-wrap { border: 1px solid #eee; padding: 20px; }
.cal-header {
    display: flex;
    flex-direction: column; /* 💡 1층 날짜, 2층 범례 */
    align-items: center;
    gap: 20px;
    margin-bottom: 30px;
    position: relative;
...
/* 트렌디한 화살표 버튼 */
.month-selector { display: flex; align-items: center; gap: 40px; }
.month-selector button {
    background: #fff; border: 1px solid #eee; border-radius: 12px; padding: 10px;
    cursor: pointer; transition: all 0.2s; display: flex; align-items: center; color: #555;
}
.month-selector button:hover { background: #f8f9fa; border-color: #005baa; color: #005baa; transform: translateY(-2px); }
.month-selector h4 { font-size: 26px; font-weight: 800; color: #333; margin: 0; }

/* 범례 (형광펜 위치) */
.cal-legend { display: flex; gap: 20px; align-self: flex-end; padding-right: 10px; }
.legend-item { display: flex; align-items: center; gap: 8px; font-size: 16px; font-weight: 700; color: #888; }
.legend-item .bar { width: 25px; height: 6px; border-radius: 2px; }

/* 달력 그리드 */
.cal-grid { display: grid; grid-template-columns: repeat(7, 1fr); background-color: #eee; gap: 1px; border: 1px solid #eee; }
.cal-day-head { background: #f8f9fa; padding: 15px; text-align: center; font-weight: 700; font-size: 16px; }
.cal-cell { background: #fff; min-height: 120px; padding: 15px; cursor: pointer; }
.cal-cell.today { background: #f0f6ff; }
.cal-cell.selected { border: 2px solid #005baa; }
.day-num { font-size: 16px; font-weight: 600; margin-bottom: 10px; display: block; color: #444; }
.cal-events { margin-top: 5px; display: flex; flex-direction: column; gap: 2px; }

/* 📌 8. 기타 유틸리티 (D-Day 등) */
.res-highlight { background: #f0f7ff; padding: 30px; text-align: center; border-radius: 8px; }
.d-day { display: inline-block; background: #005baa; color: #fff; padding: 12px 18px; font-weight: 600; font-size: 60px; margin-bottom: 20px; }
.res-time-txt { font-size: 24px; font-weight: 700; color: #444; margin-bottom: 5px; }
.res-doc-txt { font-size: 20px; font-weight: 500; color: #666; }
.status-badge { padding: 6px 14px; font-size: 14px; font-weight: 700; display: inline-block; border-radius: 4px; }
.status-badge.active { background: #e7f3ff; color: #005baa; }
.empty-msg { font-size: 20px; margin-top: 30px; text-align: center; color: #888; }
</style>