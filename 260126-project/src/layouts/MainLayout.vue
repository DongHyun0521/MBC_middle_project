      ...
      <div :class="['megamenu-panel', { active: isMegamenuOpen }]">
        <div class="megamenu-inner">
          <div class="menu-col">
            ...
            <ul><li>인사말</li><li>연혁</li><li>미션/비전</li></ul>
          </div>
          <div class="menu-col">
          ...
          <div class="menu-col">
            ...
            <ul>
              ...
              <li @click="goPage('/checkres')">예약내역 조회</li>
            </ul>
          </div>
          <div class="menu-col">
            <p class="col-title">건강정보</p>
            <ul><li>질환백과</li><li>자가진단</li></ul>
          </div>
          <div class="menu-col">
            <p class="col-title">고객서비스</p>
            <ul><li>공지사항</li><li>FAQ</li></ul>
          </div>
        </div>
      </div>
    </header>
...
const checkLogin = () => {
  ...
  if (loginData) {
    ...
    isLogin.value = true; loginName.value = user.name || user.id 
  } else { isLogin.value = false }
}

const handleLogout = () => {
  if (confirm("로그아웃 하시겠습니까? 🧩")) {
    sessionStorage.removeItem('login'); isLogin.value = false; router.push('/')
  }
}
...
.f-logo-img { height: 30px; width: auto; } /* 💡 로고 높이 빡세게 고정 ❌ */
