...
.page-desc { color: #888; font-size: 15px; font-weight: 300; }
...
.search-input { padding: 15px; border: 1px solid #ddd; border-radius: 4px; font-size: 15px; color: #4e4e4e; transition: 0.3s; background-color: #f9f9f9; }
select.search-input { width: 220px; appearance: none; background-image: url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%23999' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'%3e%3cpolyline points='6 9 12 15 18 9'%3e%3c/polyline%3e%3c/svg%3e"); background-repeat: no-repeat; background-position: right 15px center; background-size: 16px; cursor: pointer; }
.name-input { flex: 1; }
.search-input:focus { border-color: #0171e9; outline: none; background-color: #fff; box-shadow: 0 0 0 3px rgba(1, 113, 233, 0.1); }
.dept-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(450px, 1fr)); gap: 25px; }
.dept-card { background: #fff; border: 1px solid #eee; padding: 35px; border-radius: 12px; box-shadow: 0 5px 15px rgba(0,0,0,0.02); transition: all 0.3s ease; display: flex; flex-direction: column; justify-content: space-between; }
.dept-card:hover { transform: translateY(-8px); box-shadow: 0 15px 30px rgba(1, 113, 233, 0.08); border-color: #a1d8f3; }
.dept-info h3 { font-size: 22px; color: #0171e9; margin: 0 0 15px 0; font-weight: 700; }
.dept-tags { display: flex; gap: 15px; margin-bottom: 20px; }
.tag { font-size: 13px; padding: 4px 10px; border-radius: 4px; font-weight: 500; }
.location { background: #f0f7ff; color: #0171e9; }
.phone { background: #f1f5f9; color: #666; }
.desc { color: #777; font-size: 15px; line-height: 1.6; margin: 0; font-weight: 300; }
.view-doc-btn { margin-top: 30px; width: 100%; padding: 15px; background: #404347; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: 600; font-size: 15px; transition: 0.3s; }
.view-doc-btn:hover { background: #0171e9; }
.no-result { grid-column: 1 / -1; text-align: center; padding: 100px 20px; color: #999; }
</style>