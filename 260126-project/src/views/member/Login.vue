      ...
      <div class="portal-form-panel">
        ...
        <form @submit.prevent="handleLogin" class="login-form transition-box" :key="mainTab">
          ...
          <div class="regi-footer">
            <p>{{ footerText }}</p>
            <router-link to="/regi">회원가입 바로가기</router-link>
          </div>
        </form>
      </div>
    </div>
...
const handleSaveIdChange = () => {
  if (saveId.value && !confirm("개인정보 보호를 위해 본인 기기에서만 저장해 주세요")) saveId.value = false;
};

const checkID = () => { user.value.id = user.value.id.toLowerCase().replace(/[^a-z0-9]/g, ''); };

const handleFindId = async () => {
  if (!findUser.value.name || !findUser.value.email) { alert("정보를 입력해 주세요"); return; }
  try {
